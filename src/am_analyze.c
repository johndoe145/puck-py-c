        ...
        for (i = 0; i < n_samples; i++) {
                ...
                if (am_buf[i] < threshold) {
                        if (a->print2) {
                                ...
                                if (debug_output) {
                                        fprintf(stderr,
                                                "pulse_end  [%u] found at sample %u, pulse length = %u, pulse avg length = %u\n",
                                                a->pulses_found, a->counter, a->counter - a->pulse_start,
                                                (a->pulses_found) ? (a->pulse_avg / a->pulses_found) : 0);
                                }
                                ...
                                if (a->signal_pulse_counter >= PULSE_DATA_SIZE) {
                                        ...
                                        fprintf(stderr,
                                                "Too many pulses detected, probably bad input data or input parameters\n");
                                        return;
                                }
                        }
                        a->print = 1;
                        if (a->signal_start && (a->pulse_end + FRAME_END_MIN < a->counter)) {
                                ...
                                fprintf(stderr,
                                        "*** signal_start = %u, signal_end = %u, signal_len = %u, pulses_found = %u\n",
        ...
        if (signal_type == 1) {
                for (i = 0; i < aa->signal_pulse_counter; i++) {
                        if (signal_distance_data[i] > 0) {
                                ...
                                } else if ((signal_distance_data[i] > (a[0] + a[1]) / 2) &&
                                           (signal_distance_data[i] < (a[1] + a[2]) / 2)) {
                                        //                     fprintf(stderr, "0 [%d] %d > %d\n",i, signal_distance_data[i], (a[0]+a[1])/2);
